Landing page and release notes for diktafon, an axum app. Read the root
`AGENTS.md` first.
...
The **palette** is Signal, the app's, not a generic Apple one. Copy tokens from
`crates/diktafon/src/theme.rs`, and keep the site dark: the app is dark-native.

System font stack. Reach for modern CSS before adding markup: `clamp()`, logical
properties, `text-wrap`, `:focus-visible`, `@media (hover: hover)`,
...
- `static/diktafon-mark.svg` and `static/diktafon.webp` from
